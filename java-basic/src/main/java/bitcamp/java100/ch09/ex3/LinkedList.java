 ...
 * public void insert(int index, Object value) {
 * 
 * Box pre = head; Box aft; Box inser = new Box(); inser.value = value;
 * 
 * if (index == 0) { inser.next = head; head = inser; } else { for (int i = 0; i
 * < index - 1; i++) pre = pre.next; aft=pre.next; inser.next = aft; pre.next =
 * inser; } len++; }
 * 
 * public Object remove(int index) {
 * 
 * Box pre = head; Box aft; Box del = new Box(); if (index == 0) { head =
 * head.next; } else { for (int i = 0; i < index - 1; i++) pre = pre.next; del =
 * pre.next; aft = del.next; pre.next = aft; } len--; return del.value; }
 */
